...
ui <- dashboardPage(
  ...
  dashboardBody(
    ...
    fluidRow(
      box(id = "mapa_box",
          title = "Mapa amb la ciutat i variable indicades",
      ...
      box(id = "distribution_box",
          title = "Preu segons variable",
          plotlyOutput("distribution")  %>%
            withSpinner(color="#0dc5c1"),
          width = 6),
      box(id = "piechart_box",
          title = "Distribució segons variable",
  ...
  observeEvent(toListen(),{
    ...
    output$leaflet_var <- renderLeaflet({
      ...
      if (input$variable_representar == "room_type") {
        ...
        roomicon <- makeIcon(
          iconUrl = ifelse(df_airbnb$room_type == "Private room",
                           "https://static.thenounproject.com/png/1047372-200.png", #Habitació privada
                           ifelse(df_airbnb$room_type == "Entire home/apt",
        ...
        hosticon <- makeIcon(
          iconUrl = ifelse(df_airbnb$host_is_superhost == F,
                           "https://cdn-icons-png.flaticon.com/512/1654/1654220.png", #Host
                           "https://seeklogo.com/images/A/airbnb-superhost-logo-1E4451535F-seeklogo.com.png"), #Habitació compartida
          iconWidth = 30, iconHeight = 30,
          iconAnchorX = 15, iconAnchorY = 15)
    ...
    output$distribution <- renderPlotly({
      ...
      # Amaguem els gràfics si es representa el preu, doncs no ens interessa
    ...
    output$piechart <- renderPlotly({
      ...
      if (var_representar %in% c("dist", "realSum", "person_capacity", "metro_dist",
        ...
        df_plot <- df_airbnb %>% 
          mutate(var_bin = cut(!!as.symbol(var_representar),
                               cut_list))
        df_pie <- tibble(var_bin = levels(df_plot$var_bin))
      } else {
        df_plot <- df_airbnb %>%
          mutate(var_bin = !!as.symbol(var_representar))
        df_pie <- tibble(var_bin = unique(df_plot$var_bin))
      }
